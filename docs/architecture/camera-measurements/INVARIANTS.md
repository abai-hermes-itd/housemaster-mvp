...
14. **Live/current status never retroactively invalidates history.** A target's retirement, a formula's deprecation, a calibration's expiry, an actor's deactivation, a report's withdrawal — none of these change what any already-created historical record means, contains, or resolves to. This extends to the *rule* that determines a calibration's historical validity, not only its current eligibility: `CalibrationSession.validUntilAt` is pinned at creation from the policy in effect then, so a later change to validity-window policy can never reinterpret whether a historical `Measurement`'s calibration was valid at the time it was cited. *(HG-4/HG-5-FIX)*
...
16. **Successor chains that define current state are linear and acyclic; `SpatialTarget` lineage is acyclic but deliberately not linear.** `Measurement.correctsMeasurementId`, `DerivedMeasurement.supersedesDerivedMeasurementId`, `FormulaDefinition.supersedesFormulaVersion`, and `CalibrationSession.supersedesCalibrationSessionId` are each validated linear (at most one successor) and acyclic at write time. `SpatialTarget.replacesTargetIds` is validated acyclic and same-`buildingId`, with no self-reference, but is a lineage **graph**, not a linear chain — split (one target replaced by several) and merge (several targets replaced by one) are both allowed by design. *(HG-2/HG-4/HG-5-FIX)*
