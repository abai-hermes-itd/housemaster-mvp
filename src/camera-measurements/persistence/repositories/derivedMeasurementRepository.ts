...
export const derivedMeasurementRepository = {
  ...
  async create(record: DerivedMeasurement): Promise<DerivedMeasurementId> {
    ...
    // §"calculationScope"; see DerivedMeasurement.ts's field comment and
    // G1-05B-R finding #1). This is the single authoritative place that
    // transformation happens — whatever raw scope label the caller
    // passed in `record.calculationScope` is combined with the record's
    // own pinned input arrays here; the caller cannot bypass it by
    // pre-composing a "canonical-looking" string themselves, since this
    // function recomputes it from the actual pinned inputs regardless.
    // Non-aggregate formulas are stored exactly as the caller supplied —
    ...
    if (record.supersedesDerivedMeasurementId !== undefined) {
      ...
      // stored in its own canonical form at its own creation; compare it
      // against THIS record's freshly-computed canonical value, never
      // the caller's raw, pre-canonicalization input.
